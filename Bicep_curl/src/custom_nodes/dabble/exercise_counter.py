"""
Custom node to show keypoints and count the number of bicep curls.
"""
...
class BicepPoseParams:
    """Bicep pose parameters.
    ...
    @classmethod
    def from_dict(
        cls: Type["BicepPoseParams"], params_dict: Dict[str, Any]
    ) -> Type["BicepPoseParams"]:
        """Takes in a dictionary of parameters and returns a BicepPoseParams Dataclass.

        Note:
            One can also initialize the dataclass bicep_pose = BicepPoseParams(**params_dict).
        ...
        Returns:
            (BicepPoseParams): Dataclass with the parameters initalized from params_dict.
        """
...
class Node(AbstractNode):
    ...
    Args:
       ...
       config.exercise_name (str): Name of the exercise. Default: "bicep".
       config.keypoint_threshold (float): Ignore keypoints below this threshold. Default: 0.3.
       config.bicep_pose_params (:obj:`Dict[str, Any]`): Parameters for the bicep_pose. Default: {starting_elbow_angle: 155, ending_elbow_angle: 90}.


    Attributes:
        self.frame_count (int): Track the number of frames processed.
        self.expected_pose (str): The expected pose. Default: "down".
        self.num_bicep (float): Cumulative number of bicep. Default: 0.
        self.have_started_bicep (bool): Whether or not the biceps have started. Default: False.
        ...
        self.bicep_pose_params_dataclass (BicepPoseParams): bicep pose parameters.
    ...
    def __init__(self, config: Dict[str, Any] = None, **kwargs: Any) -> None:
        ...
        self.bicep_pose_params: Dict[str, Any]

        self.logger.info(f"Initialize Exercise Type: {self.exercise_name}!")

        self.frame_count = 0

        self.expected_pose_left = "down"
        self.expected_pose_right = "down"
        self.num_bicep_left = 0
        self.num_bicep_right = 0
        self.have_started_bicep = False
        self.elbow_angle = None

        self.global_params_dataclass = GlobalParams()
        self.bicep_pose_params_dataclass =BicepPoseParams.from_dict(
            self.bicep_pose_params
        )
    ...
    def inc_num_bicep_left(self) -> float:
        """Increments the number of push ups by 0.5 for every directional change.

        Returns:
            self.num_bicep (float): Cumulative number of push ups.
        """
        self.num_bicep_left += 0.5
        return self.num_bicep_left

    def inc_num_bicep_right(self) -> float:
        """Increments the number of push ups by 0.5 for every directional change.

        Returns:
            self.num_bicep (float): Cumulative number of push ups.
        """
        self.num_bicep_right += 0.5
        return self.num_bicep_right
    ...
    def is_up_pose(self, elbow_angle: float) -> bool:
        ...
        return (
            elbow_angle
            > self.bicep_pose_params_dataclass.starting_elbow_angle
        )
    ...
    def is_down_pose(self, elbow_angle: float) -> None:
        ...
        return (
            elbow_angle
            <= self.bicep_pose_params_dataclass.ending_elbow_angle
        )
    ...
    def count_bicep(
        ...
        if self.right_elbow and self.right_shoulder and self.right_wrist:

            right_elbow_angle = self.calculate_angle_using_dot_prod(
            ...
            if self.is_up_pose(right_elbow_angle):
                self.have_started_bicep = True

            # Check for full range of motion for the pushup
            if self.have_started_bicep:
                # the two if-statements are mutually exclusive: won't happen at the same time.
                if (
                    self.is_down_pose(right_elbow_angle)
                    and self.expected_pose_right == "down"
                ):
                    self.inc_num_bicep_right()
                    self.expected_pose_right = "up"

                if (
                    self.is_up_pose(right_elbow_angle)
                    and self.expected_pose_right == "up"
                ):
                    self.inc_num_bicep_right()
                    self.expected_pose_right = "down"

            bicep_str = f"#Left = {self.num_bicep_right}"
            draw_text(
                img,
                bicep_str,
                (20, 30),
                color=self.global_params_dataclass.GREEN,
                fontFace=cv2.FONT_HERSHEY_SIMPLEX,
                fontScale=1,
                thickness=3,
            )
        ...
        if self.left_elbow and self.left_shoulder and self.left_wrist:
            ...
            if self.have_started_bicep:
                ...
                if (
                    ...
                    and self.expected_pose_left == "down"
                ):
                    self.inc_num_bicep_left()
                    self.expected_pose_left = "up"

                if (
                    self.is_up_pose(left_elbow_angle)
                    and self.expected_pose_left == "up"
                ):
                    self.inc_num_bicep_left()
                    self.expected_pose_left = "down"

            bicep_str = f"Right = {self.num_bicep_left}"
            draw_text(
                img,
                bicep_str,
                (400, 30),
    ...
    def run(self, inputs: Dict[str, Any]) -> Dict[str, Any]:  # type: ignore
        ...
        Returns:
            outputs (dict): Dictionary with keys
                ...
                - num_bicep: The number of cumulative push ups.
        ...
        return {
            "filename": filename,
            "expected_pose_left": self.expected_pose_left,
            "expected_pose_right": self.expected_pose_right,
            "num_bicep_left": self.num_bicep_left,
            "num_bicep_right": self.num_bicep_right,
            "frame_count": self.frame_count,
            "elbow_angle": self.elbow_angle,
            "elbow_keypoint": self.right_elbow,
            "shoulder_keypoint": self.right_shoulder,
            "wrist_keypoint": self.right_wrist,
        }
